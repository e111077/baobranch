...
function evolveImpl(options: ArgumentsCamelCase<EvolveOptions>): void {
  ...
  if (status && options.continue) {
    ...
    const branchToResume = execCommand(`git branch --points-at ${tagToResume}`).trim();
...
function evolveChain({
  ...
  const nextBranch = execCommand(`git branch --points-at ${generateEvolveTag(step, scope)}`).trim();
